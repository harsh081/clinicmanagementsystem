        ...
        <div class="container">
            <div class="row main">
                <div class="main-login main-center">
                    <h2 style="text-align: center" > Month wise Total Patient SUMMARY REPORT</h2>
                    <form class="" method="post" >



                        <div class="form-group">
                            <label for="username" class="cols-sm-2 control-label">From Date:</label>
                            <div class="cols-sm-10">
                                <div class="input-group">
                                    <span class="input-group-addon"><i class="fa fa-users fa" aria-hidden="true"></i></span>
                                    <input type="date" class="form-control" name="FromDate" id="staffdate" />
                                </div>
                            </div>
                        </div>
                        <div class="form-group">
                            <label for="username" class="cols-sm-2 control-label">To Date:</label>
                            <div class="cols-sm-10">
                                <div class="input-group">
                                    <span class="input-group-addon"><i class="fa fa-users fa" aria-hidden="true"></i></span>
                                    <input type="date" class="form-control" name="ToDate" id="staffdate" />
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="panel-body">
                        <div class="col-md-12 no-padding">
                            ...
                            if (isset($_POST['BtnSubmit'])) {
                                ...
                                $query = $mysqli->query("select count(Patient_id) as a from appointment_master where Appointment_Date  between '$from' and '$to';");
                                if ($query == true) {
                                    while ($row = $query->fetch_assoc()) {
                                       $counts=$row['a']; 
                                    }
                                }
                                $query = $mysqli->query("select b.Full_Name as a,count(a.Patient_id) as b from appointment_master a, patient_master b where a.Patient_id=b.Patient_id and a.Appointment_Date  between '$from' and '$to' group by a.Patient_id having count(a.Patient_id) > 1");
                                if ($query == true) {
                                    while ($row = $query->fetch_assoc()) {
                                        echo '<div class="row progress-labels">
                            <div class="col-sm-6">' . $row['a'] . '</div>
                            <div class="col-sm-6" style="text-align: right;">' . round(($row['b']*100)/$counts) . '%</div>
                        </div>
                        <div class="progress">
                            <div data-percentage="0%" style="width: ' . round(($row['b']*100)/$counts) . '%;" class="progress-bar progress-bar-blue" role="progressbar" aria-valuemin="0" aria-valuemax="100"></div>
                        </div>';
                                    }
                                }
                            }
